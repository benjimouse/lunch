...
DateFormat = function () {
    ...
    this.showDateTimeForEditing = function (dateToFormat) {
        var pad = function (str, max) {
            str = str.toString();
            return str.length < max ? pad("0" + str, max) : str;
        };
        return dateToFormat.getFullYear() + '-' + pad(dateToFormat.getMonth() + 1, 2) + '-' + pad(dateToFormat.getDate(), 2) + 'T' + dateToFormat.getHours() + ':' + pad(dateToFormat.getMinutes(), 2);
    };
};
Time = function () {
    'use strict';
